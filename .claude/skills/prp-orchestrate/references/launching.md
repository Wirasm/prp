...
- Respect the run's configured `--max-parallel` (default 10, replaceable by the user at any time). Pace actual launches against current harness capacity, reserving two slots beyond the root and active delivery owners for a delivery's stage coordinator and leaf specialist. A lower effective limit or rejected spawn queues work without changing the configured maximum; completion notifications free slots, so retry queued work then.
...
Agent-tool worktrees are auto-removed when their owner is released and the checkout is unchanged; verify rather than assume. For worktrees created via `prp-worktree`, use the same skill to remove the checkout while retaining the branch; its rails refuse dirty worktrees:
