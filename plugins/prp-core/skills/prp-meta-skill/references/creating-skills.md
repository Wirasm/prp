...
- **Invocation:** omit `user-invocable`/`disable-model-invocation` so an active PRP skill is both user- and agent-invocable. A deliberately manual experiment belongs under `.claude/skills/in-process/`, sets `disable-model-invocation: true`, and is not distributed or called by compositions.
