...
Agent-tool worktrees are auto-removed when their owner is released and the checkout is unchanged; verify rather than assume. For worktrees created via `prp-worktree`, tear down with the same skill—its rails refuse dirty worktrees and unmerged branch deletion:
