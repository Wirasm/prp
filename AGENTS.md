...
- **`.claude/skills/`** — the skills, and the working **source of truth**. Each is a self-contained Agent Skill: a `SKILL.md` spine plus optional `references/`, `templates/`, `scripts/`. Manual experiments stay top-level so explicit invocation remains discoverable; register them in `PLUGIN_EXCLUDED_SKILLS` in `sync_plugin.py` to keep them out of the plugin other people install. They still render to `.agents/skills/`, because that target is the author's own Codex install rather than a public distribution.
- **`~/.prp/<key>/`** — per-project artifacts and runtime state, stored out-of-repo and shared across the project's worktrees. Artifact-writing skills resolve it with the canonical inline convention block; `$PRP_HOME` overrides the store root. The block adopts the store whose `project.json` already records this project root, whatever key minted it, and hashes the root only when no store claims it: an older or foreign resolver then converges on the existing store instead of forking a second one. `sync_plugin.py` fails the sync when any copy of the block drifts.
...
- **`.agents/skills/`, `.codex/agents/`** — the **OpenAI Codex CLI render** of the pack, also generated by `scripts/sync_plugin.py` — never edit directly. Skills minus the Claude-only ones (`prp-research-team`; `prp-orchestrate` and `prp-meta-skill` *are* rendered — orchestrate's delegation mechanics rewrite to harness-agnostic language, meta-skill's authored-skill paths map to `.agents/skills`), with Claude-isms rewritten (Task-tool dispatch → "spawn the X subagent", `prp-core:` namespace dropped, `/prp-x` → `$prp-x` mentions, an Arguments note replacing `$ARGUMENTS` substitution); agents converted to Codex custom-agent TOML.
  - **Codex reads `$CODEX_HOME/skills`** (`~/.codex/skills`), *not* `~/.agents/skills` — in Codex, `.agents/` is the plugin location (`~/.agents/plugins/marketplace.json`). Verified against codex-cli 0.147.0; skills surface in the `@` picker, and `$prp-x` is how Codex refers to one in prose. Install globally by symlinking **each** rendered skill into `~/.codex/skills/` (per-skill, because that directory is shared with every other Codex skill), and `~/.codex/agents` → `.codex/agents` for the subagents. Symlinks track every sync; copies go stale silently.
  - **Anything dropped into `.agents/skills/` that `.claude/skills/` did not generate is deleted** by `sync_plugin.py`'s stale prune. Other tools do install skills there (`~/.agents/skills` is a cross-tool convention some of them use), so keep this directory exclusively prp's; the `.gitignore` rule (`.agents/skills/*` with `!.agents/skills/prp-*`) is the backstop against committing a guest, not against losing one.
