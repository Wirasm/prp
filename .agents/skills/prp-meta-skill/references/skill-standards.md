...
For active PRP skills the default is **both** invocation paths: omit `user-invocable` and `disable-model-invocation`. A deliberately manual experiment under `.agents/skills/in-process/` may set `disable-model-invocation: true` and must remain excluded from generated distributions and composition callers.
