...
For PRP skills the default is **both** invocation paths: omit `user-invocable` and `disable-model-invocation`. Set `disable-model-invocation: true` when agent invocation is meaningless rather than merely unwanted: a skill that acts on the conversation itself, such as restating the previous response in plainer words, only works when a person asks for it, so its description is a picker label rather than a trigger and carries no trigger phrases. A deliberately in-process experiment is authored in this repository's `.claude` skill source tree, is registered in `PLUGIN_EXCLUDED_SKILLS` so it never reaches the plugin other people install, and uses only `This is an experimental skill. Never use it unless the user explicitly tells you to invoke /<name>.` as its description. It still renders to `.agents/skills/` so the author can exercise it in Codex. This keeps explicit agent delegation possible without advertising normal trigger phrases; the experiment stays out of the public plugin and out of composition callers.
