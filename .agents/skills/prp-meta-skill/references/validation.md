...
- For active PRP skills: `user-invocable` and `disable-model-invocation` are NOT set. A manual experiment under `.agents/skills/in-process/` may set `disable-model-invocation: true`; verify it is absent from generated distributions and active composition callers.
