...
2. Merge strictly one at a time. After each merge, bring remaining branches onto the new base — prefer messaging the owning agent ("rebase onto <base>, resolve, re-run validations, push") so its context handles the conflicts; rebase directly only for trivial cases. Re-check `gh pr checks <n> --required` before the next merge, or rerun the local gate when no required checks exist.
