...
description: Research codebase questions using parallel agents - documents what exists, not what should change. Use when the user asks how the codebase works, where something lives, or invokes the prp-codebase-question skill.
---
...
**Launch agents in parallel by working through them inline, one after another.**
...
To dig deeper: `the prp-codebase-question skill --follow-up {topic}`
To include external docs: `the prp-codebase-question skill --web {topic}`
```
...
the prp-codebase-question skill how does authentication work

# Include external documentation
the prp-codebase-question skill --web how does the PRP runner execute commands

# Follow up on previous research
the prp-codebase-question skill --follow-up what error handling patterns exist in the runner

# Locate and document a specific area
the prp-codebase-question skill where are all the command templates and how are they structured
```
