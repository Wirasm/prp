...
When implementation is green, invoke `the prp-commit skill` for only the work completed from this plan. Record the resulting commit SHA in the plan's append-only Lifecycle section when present and in the report.
